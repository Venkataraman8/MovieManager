...
echo"<input type='text' name='movie' size=50/>";
...
$details=array(array(array()));
	...
	for($j=0;$j<20;$j++)
	{
$url="https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=bd6253e44a43d70f2afc69233f68ebb9&page=".$pageno;
						...
						function(response)
						{
							...
							if(response && response.items) 
							{
								...
								if(items.length>0) 
								{
									...
									'<input type=button style=\'position:relative;left:310px; background-color:".$lcolor.";\' value=Like class=like id=like_{$details["results"][$j]['id']} onclick=action(\'like_{$details["results"][$j]['id']}\') />'+
									'<input type=button style=\'position:relative; left:330px; background-color:".$fcolor.";\' value=Favourite class=favourite id=favourite_{$details["results"][$j]['id']} onclick=action(\'favourite_{$details["results"][$j]['id']}\') />'+
									'<input type=button style=\'position:relative; left:350px; background-color:".$wcolor.";\' value=Watched class=watched id=watched_{$details["results"][$j]['id']} onclick=action(\'watched_{$details["results"][$j]['id']}\') />'+
									'<center>'+video+'</center>';
